...
config.accounts.forEach(account => {
setInterval(function(){sendRequest(account)},1_440_000)
})
			...
			for (const inbound of inbounds.slice(0,5)) {
				if (checked[inbound.id]==undefined) {
					...
					if (!calculatedValues.failed && calculatedValues.get < config.selfeval) {
						...
						if (calculatedValues.get < calculatedValues.give) {
							let items = await getInventory(trade.user.id, cookie);
                            logToFile(items)
							logToFile("Loaded inventory of " + trade.user.name)
							for (let b = 0; b < items.length; b++) {
								let i = items[b];
								if (othersValues[i.assetId] == undefined || config.avoid[i.assetId] || othersValues[i.assetId][4] >= config.selfeval || othersValues[i.assetId][9] != -1 || othersValues[i.assetId][3] == -1  || othersValues[i.assetId][2]<config.underrapratio*othersValues[i.assetId][4] || i.isOnHold!=false) {
									delete items[b];
								}
							}
                            ...
                            givingItems=givingItems.filter(item => myValues[item]!=undefined);
                           sortByAssetId(givingItems,myValues)

                           receivingItems=receivingItems.filter(item => othersValues[item]!=undefined);
							...
							if (true) {
								for (item of ((items.slice(0, 20)))) {
									...
									for (set of valuedCombinations) {
										if ( (set.value - targetValue <=config.upgmaxop) && set.value >= targetValue && set.value - targetValue <= ((set.value / 50) * (set.combination.length - 2))) {
											logToFile("Trade Found With " + inbound.user.name)

											let assets = []
											let send = true;
											for (offeredItem of set.combination) {
												if (myValues[offeredItem.assetId][4] >= 0.9 * myValues[item.assetId][4]) {
													send = false
												}
